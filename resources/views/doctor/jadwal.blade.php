                ...
                <div class="mt-3 col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">Jadwal Dokter {{ $doctor->user->name }} </h3>
                        ...
                        <div class="card-body">
                            @if($schedules->isEmpty())
                                <p class="text-center">Tidak ada jadwal tersedia untuk dokter ini.</p>
                            @else
                                    ...
                                    <tbody>
                                        <tr>
                                            @foreach(range(1, 7) as $hari)
                                                <td>
                                                    @php
                                                        // Filter jadwal berdasarkan hari (angka)
                                                        $daySchedules = $schedules->where('hari', $hari);
                                                    @endphp
                                                    ...
                                                    @else
                                                        @foreach($daySchedules as $schedule)
                                                            <div>
                                                                <strong>Jam:</strong> {{ $schedule->start }} - {{ $schedule->end }}<br>
                                                                <strong>Quota:</strong> {{ $schedule->total_quota }}
                                                            </div>
                                                            <hr>
                                                        @endforeach
                                                    @endif
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="addFormpermesion" action="{{ route('dokter.index.jadwal.add', ['id' => $doctor->id]) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div>
                            <div class="row">
                                <!-- Kolom Hari -->
