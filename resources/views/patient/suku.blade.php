...
@section('content')
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="nama_suku" name="nama_suku" value="{{ old('nama_suku') }}">
                                </div>
                                @error('nama_suku')
                                    <small>
                                        <div id="error-nama_suku" style="color: red;">{{ $message }}</div>
                                    </small>
                                @enderror
                            </div>
                        </div>
    ...
    <div class="modal fade" id="deleteModal{{ $delete->id }}" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Apakah Anda yakin ingin menghapus Suku ini?
                </div>
                <div class="modal-footer">
                    <form action="{{ route('datmas.suku.delete', $delete->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
    ...
    <div class="modal fade" id="editModal{{ $edit->id }}" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('datmas.suku.update', $edit->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="form-group">
                                    <label>Nama Suku</label>
                                    <input type="text" class="form-control" name="nama_update" value="{{ $edit->nama_suku }}" required>
                                </div>
                            </div>
                        </div>
                        <div class="modal-footer">
    ...
    <script>
        $(document).ready(function() {
            $('#nama_suku').on('input', function() {
                if ($(this).val().trim().length > 0) {
                    $('#error-nama_suku').fadeOut(); // Hilangkan error jika ada input
                }
            });
                    ...
                    $.ajax({
                        ...
                        error: function(xhr) {
                            if (xhr.status === 422) { // Validasi gagal (Laravel)
                                ...
                                if (errors.nama_suku) {
                                    $('#error-nama_suku').remove(); // Hapus error lama jika ada
                                    $('#nama_suku').after('<div id="error-nama_suku" style="color: red;">' + errors.nama_suku[0] + '</div>');
                                }
